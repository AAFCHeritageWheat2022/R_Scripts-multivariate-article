# ============================================================
# FIGURE 6: Grain Yield Across Years by STI Group
# ============================================================
...
# Remove NA values
data_long <- data_long[!is.na(data_long$yield), ]
...
y_max <- max(TM$yield + TM$sd, na.rm = TRUE)
...
TM <- TM %>%
  ...
  dplyr::mutate(
    ...
    # Yield values inside bars
    value_y_pos = yield * 0.5
  ) %>%
  dplyr::ungroup()
...
# ============================================================
# PART 7: CREATE FINAL PLOT (WITH LEGEND ON RIGHT)
# ============================================================
...
final_plot <- ggplot(data = TM, aes(x = specgroup, y = yield, fill = specgroup)) +
  ...
  # Add yield values inside bars
  geom_text(aes(label = sprintf("%.0f", yield), y = value_y_pos), 
            position = position_dodge(0.8), 
  ...
  # Labels
  labs(
    title = "Grain Yield Across Years by STI Group",
    x = "STI Group",
    y = "Grain Yield (kg/ha)",
    fill = "STI Group"
  ) +
...
# Save the plot
save_publication_formats(final_plot, "Figure6_Grain_Yield_Analysis", width = 16, height = 8)
