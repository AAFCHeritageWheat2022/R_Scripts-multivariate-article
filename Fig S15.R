...
# Remove NA values
data_long <- data_long[!is.na(data_long$e_apparent), ]
...
y_max <- max(TM$e_apparent + TM$sd, na.rm = TRUE)
...
TM <- TM %>%
  ...
  dplyr::mutate(
    ...
    # E_apparent values inside bars
    value_y_pos = e_apparent * 0.5
  ) %>%
  dplyr::ungroup()
...
final_plot <- ggplot(data = TM, aes(x = specgroup, y = e_apparent, fill = specgroup)) +
  ...
  # Add e_apparent values inside bars
  geom_text(aes(label = sprintf("%.2f", e_apparent), y = value_y_pos), 
            position = position_dodge(0.8), 
  ...
  # Labels
  labs(
    title = expression("E"[apparent]~" (Apparent Photosynthetic Rate) Across Years by STI Group"),
    x = "STI Group",
    y = expression(paste("E"[apparent], " (mmol m"^{-2}, " s"^{-1}, ")")),
    fill = "STI Group"
  ) +
...
# Save the plot
save_publication_formats(final_plot, "FigureS11_Eapparent_Analysis", width = 14, height = 7)
