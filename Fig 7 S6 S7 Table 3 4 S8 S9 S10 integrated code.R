...
# ============================================================
# FIGURE 7: COMPLETE REGRESSION ANALYSIS - INTEGRATED
# MLR, SVM, MERF, XGBoost with SHAP Analysis and Feature Importance
# ALL PREDICTORS INCLUDING ENVIRONMENTAL VARIABLES (VIF >= 10)
...
df <- read.csv("Figure7a_for regression analyses.csv", header = TRUE)
...
# Convert newspecgroup to factor with proper levels
df$newspecgroup <- factor(df$newspecgroup, 
...
metadata_cols <- c("year", "newspecgroup", "uavspecgroup", "group", "entry", "variety")
...
env_vars <- predictor_cols[grep("Soil|air|VPD|Rainfall|temp|Cumm|NDVI|CCI", predictor_cols, ignore.case = TRUE)]
...
create_prediction_plot <- function(observed, predicted, model_name, test_year, train_years, color_var) {
  ...
  ggplot(plot_df, aes(x = Observed, y = Predicted, color = Group)) +
    ...
    # REMOVED: geom_abline(intercept = 0, slope = 1, linetype = "dashed", color = "gray50", linewidth = 0.8) +
...
export_mlr_table <- function(mlr_model, test_year, train_years, model_name = "MLR") {
  ...
  cat("\nPredictors retained in final model:", nrow(df_table_clean), "\n")
  cat(paste(rep("-", 80), collapse = ""), "\n")
  ...
  for(model_name in names(models)) {
    if(!is.null(models[[model_name]])) {
      ...
      plot <- create_prediction_plot(test_data$GY, preds, model_name, test_year, 
                                     train_years, test_data$newspecgroup)
